...
export default function DosClassManagementPage({ t, onBack }) {
    ...
    const [classForm, setClassForm] = useState({ name: '', academicYear: '2025 / 2026', subjects: [emptySubject()] });
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const [classResponse, subjectResponse, teacherResponse, assignmentResponse] = await Promise.all([api.get('/classes'), api.get('/subjects'), api.get('/teachers'), api.get('/teacher-assignments')]);
    ...
    const createClass = async (event) => {
        ...
        try {
            const response = editingClass
                ? await api.put(`/classes/${editingClass.id}`, { name: classForm.name, academicYear: classForm.academicYear })
                : await api.post('/classes', { name: classForm.name, academicYear: classForm.academicYear });
